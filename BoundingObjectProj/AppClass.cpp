...
void AppClass::InitVariables(void)
{
	...
	m_pBOMngr = BoundingObjectManager::GetInstance();
	

	m_pBOMngr->AddObject(MyBoundingObjectClass(m_pMeshMngr->GetVertexList("Zombie"), 0));
	m_pBOMngr->AddObject(MyBoundingObjectClass(m_pMeshMngr->GetVertexList("Steve"), 0));
	m_pBOMngr->AddObject(MyBoundingObjectClass(m_pMeshMngr->GetVertexList("Cow"), 0));
...
void AppClass::Update(void)
{
	...
	m_pBOMngr->objectList[0].m_v3Position = m_v3Position;
	m_pBOMngr->objectList[0].SetModelMatrix(m_pMeshMngr->GetModelMatrix("Zombie"));

	m_pMeshMngr->SetModelMatrix(mTranslation, "Steve");
	m_pBOMngr->objectList[1].m_v3Position = v3Current;
	m_pBOMngr->objectList[1].SetModelMatrix(m_pMeshMngr->GetModelMatrix("Steve"));

	m_pBOMngr->objectList[2].m_v3Position = vector3(2.5, 2.0, 0.0);
	m_pBOMngr->objectList[2].SetModelMatrix(m_pMeshMngr->GetModelMatrix("Cow"));
	...
	m_pMeshMngr->PrintLine(std::to_string(m_pBOMngr->objectList[0].GetRadius()), RERED);
	m_pMeshMngr->Print("Center: (");
	m_pMeshMngr->Print(std::to_string(m_pBOMngr->objectList[0].GetCenterGlobal().x), RERED);
	m_pMeshMngr->Print(" , ");
	m_pMeshMngr->Print(std::to_string(m_pBOMngr->objectList[0].GetCenterGlobal().y), RERED);
	m_pMeshMngr->Print(" , ");
	m_pMeshMngr->Print(std::to_string(m_pBOMngr->objectList[0].GetCenterGlobal().z), RERED);
